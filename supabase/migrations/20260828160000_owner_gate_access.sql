-- Owner gate access without invitation pass.
-- access_logs may reference a resident profile instead of an invitation.
...
alter table public.access_logs
  drop constraint if exists access_logs_subject_check;

alter table public.access_logs
  add constraint access_logs_subject_check check (
    (
...
create index if not exists access_logs_profile_property_ts_idx
  on public.access_logs (profile_id, property_id, timestamp desc);

create index if not exists access_logs_property_id_idx
  on public.access_logs (property_id);
  ...
  using (
    ...
    or id in (
      ...
      where al.profile_id is not null
        and al.gate_id in (select public.active_shift_gate_ids())
    )
    or (
